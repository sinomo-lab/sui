...
fn bindings_generate(check: bool) -> Result<(), String> {
    ...
    let python_stub_template = fs::read_to_string(&python_stub_template_path).map_err(|error| {
        format!(
    ...
    if check {
        ...
        println!("generated {}", relative_display(&root, &js_api_path));
        println!("generated {}", relative_display(&root, &python_stub_path));
    }
    ...
    if language == "python" {
        ...
        for item in items {
            match item.python_kind.as_deref() {
                Some("class") => {
                    output.push_str("    m.add_class::<Py");
                    output.push_str(&item.name);
                    output.push_str(">()?;\n");
                }
                Some("function") => {
                    let rust_name = item
                        .python_rust
                        .as_deref()
                        .expect("validated Python function has Rust name");
                    output.push_str("    m.add_function(wrap_pyfunction!(");
...
fn render_idiomatic_ts_factory(output: &mut String, factory: &TsFactorySignature) {
    if !factory.optional.is_empty() {
        ...
        for parameter in &factory.optional {
            output.push_str("  ");
    ...
    output.push_str("export function ");
    output.push_str(&factory.factory_name);
    output.push('(');
    for (index, parameter) in factory.required.iter().enumerate() {
        if index > 0 {
            output.push_str(", ");
        }
        output.push_str(&parameter.name);
    ...
    if !factory.optional.is_empty() {
        ...
        output.push_str(factory_options_parameter_name(factory));
        output.push_str("?: ");
        output.push_str(&factory.legacy_name);
        output.push_str("Options");
    }
...
fn render_generated_js_api(items: &[Item]) -> Result<String, String> {
    ...
    output.push_str("// Generated by `cargo xtask bindings generate` from bindings/widgets.sui.\n");
    output.push_str("// Do not edit by hand.\n\n");
    output.push_str("function decorateApi(native) {\n");
    output.push_str("  const factories = {\n");
    for factory in &factories {
        let options_name = factory_options_parameter_name(factory);
        output.push_str("    ");
        output.push_str(&factory.factory_name);
        output.push('(');
        for (index, parameter) in factory.required.iter().enumerate() {
        ...
        for parameter in &factory.required {
            if argument_index > 0 {
                output.push_str(", ");
            }
            output.push_str(&parameter.name);
...
fn factory_options_parameter_name(factory: &TsFactorySignature) -> &'static str {
    if factory
        .required
        .iter()
        .any(|parameter| parameter.name == "options")
    {
        "config"
    } else {
        "options"
...
fn to_lower_camel_case(value: &str) -> String {
    if value == "Switch" {
        // `switch` is valid as an object property but not as a TypeScript export binding.
        return "switchControl".to_owned();
    }
...
"#,
    ...
    #[test]
    fn language_factory_names_follow_host_conventions() {
        assert_eq!(to_snake_case("VirtualScrollView"), "virtual_scroll_view");
        assert_eq!(
            to_lower_camel_case("VirtualScrollView"),
            "virtualScrollView"
        );
        assert_eq!(to_lower_camel_case("Switch"), "switchControl");
    }
